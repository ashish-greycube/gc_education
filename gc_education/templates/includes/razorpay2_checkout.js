...
frappe.provide('razorpay2');

razorpay2.make_payment_log = function(response, options, doctype, docname, token){
	$('.razorpay-loading').addClass('hidden');
	$('.razorpay-confirming').removeClass('hidden');

	frappe.call({
		method:"gc_education.templates.pages.razorpay2_checkout.make_payment",
		freeze:true,
