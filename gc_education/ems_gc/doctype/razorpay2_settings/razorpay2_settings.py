...
class Razorpay2Settings(Document):
    ...
    def validate(self):
        create_payment_gateway("Razorpay2")
        call_hook_method("payment_gateway_enabled", gateway="Razorpay2")
        ...
        try:
            ...
            for addon in kwargs.get("addons"):
                ...
                if not resp.get("id"):
                    frappe.log_error(
                        message=str(resp),
                        title="Razorpay Failed while creating subscription",
                    )
        except Exception:
            frappe.log_error()
            # failed
    ...
    def get_payment_url(self, **kwargs):
        integration_request = create_request_log(kwargs, service_name="Razorpay2")
        return get_url(f"./razorpay2_checkout?token={integration_request.name}")
...
def capture_payment(is_sandbox=False, sanbox_response=None):
    ...
    """
    controller = frappe.get_doc("Razorpay2 Settings")

    for doc in frappe.get_all(
        "Integration Request",
        filters={"status": "Authorized", "integration_request_service": "Razorpay2"},
        fields=["name", "data"],
    ):
        try:
            if is_sandbox:
...
def razorpay_subscription_callback():
    try:
        ...
        data.update({"payment_gateway": "Razorpay2"})
